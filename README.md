...
### Training the model
```python
learn = create_cnn(data, arch=arch, metrics=metrics, wd=wd)  # weight decay
```
This is the model's architecture after training:
```
Sequential(
  (0): Sequential(
...
We can plot the confusion matrix and the algorithm's worse predictions for the validation set.
...
```
<img src="https://github.com/mlsmall/Hispathologic-Cancer-Detection/blob/master/confusion%20matrix.png" width='300' />
